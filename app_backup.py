...
import streamlit as st
from main import calculate_net_salary  # لازم تكون دي موجودة في main.py
...
# --- الراتب الأساسي ---
st.header("الراتب الأساسي")
base_salary = st.number_input("الراتب الأساسي (ريال)", min_value=0.0, format="%.2f")

# --- البدلات ---
st.header("البدلات")
allowances = {
    "بدل موقع": st.number_input("بدل موقع", min_value=0.0, format="%.2f"),
    "بدل وجبات": st.number_input("بدل وجبات", min_value=0.0, format="%.2f"),
    "بدل إجازات": st.number_input("بدل إجازات", min_value=0.0, format="%.2f"),
    "بدل تذاكر": st.number_input("بدل تذاكر", min_value=0.0, format="%.2f"),
    "مكافأة": st.number_input("مكافأة", min_value=0.0, format="%.2f"),
    "حوافز": st.number_input("حوافز", min_value=0.0, format="%.2f"),
}

# --- الخصومات ---
st.header("الخصومات")
deductions = {
    "غياب": st.number_input("خصم غياب", min_value=0.0, format="%.2f"),
    "جزاءات": st.number_input("جزاءات", min_value=0.0, format="%.2f"),
    "سلف": st.number_input("سلف", min_value=0.0, format="%.2f"),
}

# --- حساب الراتب ---
if st.button("احسب الراتب الصافي"):
    net_salary = calculate_net_salary(base_salary, allowances, deductions)
    st.success(f"💰 الراتب الصافي: {net_salary:.2f} ريال")
